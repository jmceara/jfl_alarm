...
Four services, and each exists because it is genuinely not an entity:
...
* `send_raw_command` — the reverse-engineering tool. **Admin only**, and the one place in the
  integration that can send something nobody has verified. It lives in its own module,
  `raw_command.py`, and is registered **only when that module is present**, so a distribution that
  should not offer it simply ships without the file. An action that hands arbitrary bytes to an
  alarm panel is not something every audience should be given.
...
try:
    from . import raw_command
except ImportError:  # pragma: no cover - true only where the module is deliberately not shipped
    # Not an error and not a degraded mode. Withholding an action by leaving one file out is the
    # whole point of the seam; everything else in this module is unaffected.
    raw_command = None  # type: ignore[assignment]
...
def async_register_services(hass: HomeAssistant) -> None:
    ...
    if raw_command is not None:
        hass.services.async_register(
            DOMAIN,
            raw_command.SERVICE_SEND_RAW_COMMAND,
            _make_handler(hass, raw_command.async_send_raw_command),
            schema=raw_command.schema(_TARGET_SCHEMA),
            supports_response=SupportsResponse.OPTIONAL,
        )
