...
MIN_STATUS_INTERVAL: Final = 5
MAX_STATUS_INTERVAL: Final = 600
...
RAW_REPLY_WINDOW: Final = 1.0
"""Seconds to collect frames after a raw command, for the `send_raw_command` service response.

Path-A commands answer with a status frame and not with a correlated acknowledgement, so "the reply"
to an unknown command is honestly a window in time rather than a response. One second is what the
panel took to answer every captured command.
"""
