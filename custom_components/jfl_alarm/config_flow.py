...
def _test_bind(host: str, port: int) -> None:
    """Try to bind *host*:*port*, raising `OSError` if something else already has it.

    Run in an executor: `socket.bind` is a blocking call, and a blocking call in the event loop is
    exactly what Home Assistant's own checks complain about.
    """
