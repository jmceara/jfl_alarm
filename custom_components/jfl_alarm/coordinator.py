...
class JflProgramming:
    ...
    parser never returns the code itself — see `protocol/programming.py`. AGENTS.md §4.
    """
    ...
    Decoded 2026-08-09 from a labelled differential — see `protocol/programming.py`."""
...
class JflPanelCoordinator(DataUpdateCoordinator[JflPanelState]):
    ...
    def capabilities(self) -> JflCapabilities:
        ...
        The single place the three sources are combined — see `protocol/capabilities.py`. It is
        rebuilt on demand rather than cached because two of its three inputs change: the status
        frame arrives after the entities do, and the programming only after an explicit read. The
        merge is cheap, and a stale capability is worse than recomputing one.
        """
    ...
    async def async_send_raw(self, cmd: int, payload: bytes) -> list[str]:
        """Send an arbitrary command and return the frames the panel sent back, as hex.

        The reverse-engineering tool, and the one service that can send something nobody has
        verified — which is why it is admin-only, goes through both gates like every other command,
        and logs at `warning` rather than `debug`. The checksum and the framing are computed here,
        so the caller cannot produce a frame the panel will silently drop.

        The reply is whatever arrived in the next moment, read out of the link's ring buffer.
        Path-A commands answer with a status frame rather than an acknowledgement, so "the reply" is
        a window in time, not a correlated response.
        """
        LOGGER.warning(
            "Panel %s: sending a raw command 0x%02X with payload %s. This bypasses every check "
            "this integration makes about which commands are safe",
            self.serial,
            cmd,
            payload.hex(" ") or "(empty)",
        )
        seen_before = len(self.link.frames)
        await self._async_command(lambda seq: build_frame(seq, cmd, payload))
        await asyncio.sleep(RAW_REPLY_WINDOW)
        return [
            frame.data.hex(" ") for frame in self.link.frames[seen_before:] if not frame.outbound
        ]
