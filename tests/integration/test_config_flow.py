...
from tests.integration.conftest import LOOPBACK, free_port, make_entry
from tests.panel_sim import FakePanel


async def test_the_user_step_only_asks_for_a_port(hass: HomeAssistant, port: int) -> None:
...
async def test_an_empty_host_falls_back_to_every_interface(hass: HomeAssistant, port: int) -> None:
...
async def test_a_busy_port_is_reported_as_a_form_error(hass: HomeAssistant, port: int) -> None:
...
async def test_reconfigure_reports_a_busy_port(hass: HomeAssistant, setup_entry) -> None:
...
async def test_the_options_flow_saves_every_hub_option(hass: HomeAssistant, setup_entry) -> None:
    ...
    assert result["type"] is FlowResultType.CREATE_ENTRY
    assert setup_entry.options[CONF_STATUS_INTERVAL] == 45
...
async def test_a_panel_can_be_added_by_hand(hass: HomeAssistant, port: int) -> None:
    ...
    try:
        ...
        assert subentry.data[CONF_ZONE_POLICY] == ZONES_ALL
    finally:
...
async def test_a_named_panel_keeps_its_name(hass: HomeAssistant, port: int) -> None:
    """A name the user typed wins over the generated one."""
    entry = make_entry(port, serials=[])
    entry.add_to_hass(hass)
    assert await hass.config_entries.async_setup(entry.entry_id)
    await hass.async_block_till_done()
    ...
    try:
        ...
        result = await hass.config_entries.subentries.async_configure(
            result["flow_id"],
            {
                CONF_SERIAL: "NAMED00001",
                CONF_NAME: "Beach house",
...
async def test_an_empty_serial_is_rejected(hass: HomeAssistant, setup_entry) -> None:
    ...
    result = await hass.config_entries.subentries.async_configure(
        result["flow_id"],
        {CONF_SERIAL: "   ", CONF_NAME: "", CONF_READ_ONLY: True, CONF_ZONE_POLICY: "programmed"},
    )
    assert result["type"] is FlowResultType.FORM
    assert result["errors"] == {CONF_SERIAL: "invalid_serial"}


async def test_adding_the_same_panel_twice_aborts(hass: HomeAssistant, setup_entry) -> None:
...
async def test_a_panel_subentry_can_be_reconfigured(hass: HomeAssistant, setup_entry) -> None:
    """Read-only and the zone policy are per panel, and the serial is not editable."""
    [subentry] = list(setup_entry.subentries.values())
    result = await setup_entry.start_subentry_reconfigure_flow(hass, subentry.subentry_id)
    assert result["step_id"] == "reconfigure"
    assert CONF_SERIAL not in result["data_schema"].schema

    result = await hass.config_entries.subentries.async_configure(
        result["flow_id"], {CONF_READ_ONLY: False, CONF_ZONE_POLICY: ZONES_ALL}
    )
    await hass.async_block_till_done()
    ...
    assert updated.data[CONF_ZONE_POLICY] == ZONES_ALL
