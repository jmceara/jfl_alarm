...
from custom_components.jfl_alarm.coordinator import JflPanelCoordinator
...
def make_entry(
    ...
    *,
    ...
    return MockConfigEntry(
        ...
        options={CONF_STATUS_INTERVAL: 3600, **(options or {})},
        unique_id=str(port),
...
def announce_programming(
    ...
    *pgms* maps PGM number to function byte. A number left out has no record at all, which is what
    ...
    `tests/integration/test_capability_detection.py` covers the real path — a read served over the
    socket, placing the fence's output on the fence device — so nothing here is the only cover for
    it.
    """
